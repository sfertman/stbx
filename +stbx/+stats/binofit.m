...
% error handling
twoDimsOnly         = struct('identifier', {'stbx:stats:binofit001'}, 'message', {'This function works on data with up-to 2 dimensions only.'});
countsNotNumInts = struct('identifier', {'stbx:stats:binofit002'}, 'message', {'Trial counts must be intereg numbers.'});
countsDimsMismatch  = struct('identifier', {'stbx:stats:binofit003'}, 'message', {'Dimension mismatch between success and trial counts.'});
wrongConfidence     = struct('identifier', {'stbx:stats:binofit004'}, 'message', {'Confidence level must be a real number in (0,1).'});
