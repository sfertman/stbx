...
% <TODO>
% make it work independently of FlatTable. 
% parameter names can be defined explicitely or by name and prefix (any
% length) + param name
% </TODO>
...
if nargin == 0
    help FlatTable.FT_parseParams;
    error('no input provided, help for the file is displayed above');
end
    ...
    if ~isempty(pIdx)
        ...
        if ~isempty(paramToParse{ii}{2})
            if ~isempty(paramToParse{ii}{3})
                ...
                assert(paramToParse{ii}{2}(varargout{varsOutIdx}), 'Parameter does not qualify assertion rules.');
            end
        end
